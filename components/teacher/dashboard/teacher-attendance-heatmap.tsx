...
import { AttendanceHourlyWeek } from "@/lib/types/attendance";

const hours = ["8:00 - 10:00", "10:00 - 12:00", "12:00 - 14:00 (Break)", "14:00 - 16:00", "16:00 - 18:00"];
...
interface TeacherAttendanceHeatmapProps {
  data: AttendanceHourlyWeek[];
}

export function TeacherAttendanceHeatmap({ data }: TeacherAttendanceHeatmapProps) {
  const attendanceMap: {
      ...
      <div className="grid grid-cols-6 gap-1 text-xs">
        ...
        {hours.map(hour => (
          <div key={hour} className="contents">
            {/* Hour label */}
            <div className={`text-right pr-2 font-medium ${hour.includes("Break") ? "text-blue-500 italic" : ""}`}>
              {hour}
            </div>
            ...
            {days.map(day => {
              ...
              if (hour.includes("Break")) {
                return (
                  <div
                    key={`${hour}-${day}`}
                    className="h-12 rounded dark:bg-gray-700 bg-gray-200 flex items-center justify-center dark:text-gray-300 text-gray-500 text-xs font-medium italic"
                  >
                    Break
                  </div>
                );
              ...
              const value = attendanceMap[day]?.[hour] ?? 0;
              ...
              return (
                <div
                  ...
                  className={`h-12 rounded ${getColorIntensity(
                    value
                  )} flex items-center justify-center text-white text-xs font-medium cursor-pointer hover:scale-102 transition-transform`}

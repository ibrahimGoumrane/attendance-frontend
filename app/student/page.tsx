...
// Dummy attendance data matching the provided interface
...
export default function StudentPage() {
  ...
  const uniqueSubjects = useMemo(() => {
    const subjects = attendanceRecords.map((record) => record.subject);
  ...
  const filteredRecords = useMemo(() => {
    return attendanceRecords.filter((record) => {
      ...
      const subjectMatch =
        subjectFilter === "all" || record.subject.id === subjectFilter;
          ...
          {hasActiveFilters && (
            <div className="mt-4 flex flex-wrap gap-2">
              ...
              {subjectFilter !== "all" && (
                <Badge variant="secondary" className="flex items-center gap-1">
                  Subject:{" "}
                  {uniqueSubjects.find((s) => s.id === subjectFilter)?.name}
                  <X
                    className="h-3 w-3 cursor-pointer"
